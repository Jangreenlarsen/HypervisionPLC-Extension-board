...
>
> **Hardware-revision 2026-09-14 (2. ændring samme dag, Jan bekræftet):** MODE_SEL (GPIO4) er en INPUT, ikke en OUTPUT — Jan opdagede at boardet ved en tidligere udgave af denne revision altid rapporterede `rs485`, uanset hvad han påtrykte GPIO4 udefra, fordi pinden reelt var en firmware-styret output. Rettet: GPIO4 er nu et fysisk jumper/strap SAT VED FREMSTILLING (installatøren/fabrikanten forbinder den til 3.3V=RS485 eller GND=RS232, ÉN gang), læst af firmwaren (`INPUT_PULLUP`) ÉN gang ved boot — ikke længere noget `PUT /api/channels/{n}/config` kan sætte. Se v0.17.0 i CHANGELOG.md/FEATURES.md og BUGS.md for detaljer.
...
| Signal | GPIO | Formål |
|---|---|---|
| MODE_SEL (RS232/RS485-valg, HELE boardet) | GPIO4 | INPUT (`INPUT_PULLUP`) — fysisk jumper/strap sat VED FREMSTILLING (3.3V=RS485, GND=RS232), læst af firmwaren ÉN gang ved boot (`read_board_mode_sel()`, §3.2/modbus_channel.cpp) og gælder BEGGE kanaler; IKKE settable via REST |

**Hardware-detalje — RS485-driveren skal IKKE aktiveres af DIR alene:** DE-benet på RS485-transceiveren (SP3485) skal kun være aktiv når BÅDE `MODE_SEL=RS485` OG `DIR=send` — ellers ville et RS232-konfigureret kanal utilsigtet aktivere RS485-driveren når DIR'ens boot-default tilfældigvis er høj. Løses med en simpel 2-input AND-gate (eller en enkelt transistor) mellem MODE_SEL og hver kanals egen DIR, hvis output fodrer den pågældende RS485-transceivers DE-ben — MODE_SEL-noden (den fysiske jumper/strap, se ovenfor) fødes altså til BEGGE kanalers AND-gate (ét fælles signal, to gates); RS232-transceivernes SHDN/enable-ben fodres separat, direkte (evt. inverteret) af samme MODE_SEL-node alene — samme "kun én transceiver aktiv ad gangen"-garanti som §2.2.1 kræver.
>
> **Note (2026-09-14, 2. revision):** da MODE_SEL nu er en INPUT for ESP32'en (GPIO4 læser noden, driver den ikke længere), er der INGEN konflikt i at samme fysiske jumper/strap-node samtidig fodrer BEGGE AND-gates elektrisk OG ESP32'ens GPIO4-pin — en `INPUT_PULLUP`-konfigureret GPIO trækker ikke strøm ud på linjen, den lytter blot. AND-gate-logikken ovenfor er derfor uændret af firmware-revisionen; kun hvem der SÆTTER nodens spænding er ændret (en fysisk jumper, ikke længere en ESP32-output-pin).

**Firmware-konsekvens (revideret 2026-09-14):** `mode` er IKKE længere et felt `PUT /api/channels/{n}/config` accepterer — et evt. `"mode"`-felt i requestet ignoreres stiltiende (§4.2/PLC_INTEGRATION_MANUAL.md). `mode` optræder kun som en LÆSEVÆRDI, hardware-udlæst ved boot (`GET /api/channels/{n}`, `GET /api/status`s `board_mode`) — identisk for begge kanaler, da de deler samme fysiske MODE_SEL-node.
